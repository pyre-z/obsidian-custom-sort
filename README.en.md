...
This fork adds **i18n (internationalization) support**:

- **Simplified Chinese** and **English** are both supported
- Defaults to **auto-follow the Obsidian interface language** (switch manually in Settings → Interface language)
- Covers the whole UI: settings tab, context menus, commands, notifications, status bar, and parsing error messages
- Console error logs stay in English for easier cross-referencing with upstream docs
